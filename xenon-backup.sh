...
/usr/bin/rclone delete xenon:/Xenon-MongoDB/$(date +%d) \
--drive-use-trash=false \
--config /root/.config/rclone/rclone.conf >/dev/null 2>&1

# Dump and upload data via stdout and gzip + archive while uploading

/usr/bin/mongodump --archive --gzip --host 10.0.0.2 --port 27017 | /usr/bin/rclone rcat xenon:/Xenon-MongoDB/$(date +%d)/Dump.archive \
